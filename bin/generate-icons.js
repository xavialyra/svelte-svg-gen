...
const SCRIPT_NAME = "svg-to-svelte";
const DEFAULT_OUTPUT_DIR = "src/lib/svg-icons";
const COMPONENTS_SUBDIR = "components";
const DEFAULT_LOADER_NAME = "SvgIcon";
const DEFAULT_TYPE_NAME = "SvgName";
// --- Helper Functions ---
...
async function regenerateFilesFromComponents(
  ...
  const loaderFileName = `${loaderComponentName}.svelte`;
  ...
  try {
    ...
    console.log(
      kleur.green(`  ✔ Regenerated loader component:`),
      kleur.dim(path.relative(process.cwd(), loaderOutputPath))
    );
  } catch (error) {
    console.error(kleur.red(`  ✖ Error writing regenerated loader component file ${loaderOutputPath}:`), error);
  }
...
async function run() {
  ...
  let argv = await yargs(hideBin(process.argv))
    ...
    .option("loaderName", {
      alias: "l",
      type: "string",
      description: "Name for the dynamic loader Svelte component",
      default: DEFAULT_LOADER_NAME,
    })
    .option("typeName", {
      alias: "t",
      type: "string",
      description: "Name for the generated SVG name type union",
    ...
    .option("regenerate", {
      alias: "r",
      type: "boolean",
      description: `Regenerate ${DEFAULT_TYPE_NAME}.ts and ${DEFAULT_LOADER_NAME}.svelte from existing components in output/components`,
      default: false,
    })
    .help()
    .alias("help", "h")
  ...
  const loaderComponentName = argv.loaderName;
  const iconTypeName = argv.typeName;
  ...
  console.log(kleur.gray(`  Components Sub: `), kleur.blue(COMPONENTS_SUBDIR));
  console.log(kleur.gray(`  Loader Name:    `), kleur.blue(loaderComponentName));
  console.log(kleur.gray(`  Type Name:      `), kleur.blue(iconTypeName));
  ...
  console.log(kleur.magenta(`\nGenerating aggregate files (${iconTypeName}.ts, ${loaderComponentName}.svelte)...`));
  let existingIcons = [];
  if (!argv.clean) {
    console.log(
  ...
  const typesFileName = `${iconTypeName}.ts`;
  const typesOutputPath = path.join(outputDir, typesFileName);
  try {
    await fs.ensureDir(path.dirname(typesOutputPath));
    await fs.writeFile(typesOutputPath, typeContent, "utf-8");
